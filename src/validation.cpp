...
#if defined(NDEBUG)
# error "Bitcoin Money cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitcoin Money Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s bcm additional fees, %d delta bytes\n",
...
{   
    if(nHeight >= (consensusParams.BCMForkHeight+consensusParams.BCMPremineBlock))
        return 0;
       
     //839832089000
    if(nHeight >= consensusParams.BCMForkHeight && nHeight < (consensusParams.BCMForkHeight+consensusParams.BCMPremineBlock))
        return 16796641.78 * COIN;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (nSigOpsCost > MaxBlockSigops(std::numeric_limits<uint64_t>::max()))
...
static bool IsUAHFenabled(const Consensus::Params& consensusparams, int nHeight) {
    return nHeight >= consensusparams.BCMForkHeight;
}
...
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    do {
        ...
        bool fInitialDownload;
        {
            ...
            CBlockIndex *pindexOldTip = chainActive.Tip();//获取当前链的最高块
            if (pindexMostWork == nullptr) {
                pindexMostWork = FindMostWorkChain();//获取工作量最多的链，表示区块链最长的，处理交易最多的一条链
            }

            // Whether we have anything to do at all.
            if (pindexMostWork == nullptr || pindexMostWork == chainActive.Tip())//如果没有找到工作量最多的链或者找到的链和当前的最高块一样，则返回
                return true;

            bool fInvalidFound = false;
            std::shared_ptr<const CBlock> nullBlockPtr;
            if (!ActivateBestChainStep(state, chainparams, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace))//连接到最长的链并验证状态
                return false;
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Size limits
    if (block.vtx.empty() || block.vtx.size() * WITNESS_SCALE_FACTOR > MaxBlockSize(std::numeric_limits<uint64_t>::max())  || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) * WITNESS_SCALE_FACTOR > MAX_BLOCK_WEIGHT)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // failed).
    if (GetBlockWeight(block) > MaxBlockSize(std::numeric_limits<uint64_t>::max())) {
...
static bool TryAcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, 
CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile blkdat(fileIn, 2*MaxBlockSerializedSize(std::numeric_limits<uint64_t>::max()), MaxBlockSerializedSize(std::numeric_limits<uint64_t>::max())+8, SER_DISK, CLIENT_VERSION);
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // read size
                blkdat >> nSize;
                if (nSize < 80 || nSize > MaxBlockSerializedSize(std::numeric_limits<uint64_t>::max()))
                    continue;
